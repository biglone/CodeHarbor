...
import {
  buildAutoDevSecondaryReviewReceiptTemplate,
} from "./autodev-secondary-review-receipt";
...
export function buildAutoDevSecondaryReviewHandoffNotice(
  ...
  const receiptTemplate = buildAutoDevSecondaryReviewReceiptTemplate({
...
- workflowDiagRunId: ${input.workflowDiagRunId}
- workdir: ${input.workdir}
- protocol: AUTODEV_SECONDARY_REVIEW_RECEIPT/v1
...
${receiptTemplate}`,
  ...
  return {
    ...
    diagMessage: `secondaryReview target=${target} task=${input.task.id} completionGate=${
      input.completionGatePassed ? "passed" : "failed"
    } requestId=${input.requestId} runId=${input.workflowDiagRunId} protocol=receipt_v1`,
  };
}
