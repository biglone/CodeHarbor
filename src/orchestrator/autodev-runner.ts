...
interface AutoDevRunnerDeps {
  ...
  handleAutoDevLoopStopIfRequested: (input: AutoDevLoopStopCheckInput) => Promise<boolean>;
...
export async function runAutoDevCommand(
  ...
  if (!requestedTaskId) {
    ...
    try {
      while (true) {
        const shouldStopLoop = await deps.handleAutoDevLoopStopIfRequested({
...
- remaining: pending=${summary.pending}, in_progress=${summary.inProgress}, blocked=${summary.blocked}, cancelled=${summary.cancelled}`,
        ...
        const shouldStopBeforeNextTask = await deps.handleAutoDevLoopStopIfRequested({
          sessionKey: input.sessionKey,
          conversationId: input.message.conversationId,
          loopStartedAt,
          attemptedRuns,
