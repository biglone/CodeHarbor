...
| 任务ID | 任务描述 | 预估时间 | 优先级 | 依赖 | 状态 |
|--------|----------|----------|--------|------|------|
| T6.1 | 对外触发：新增 API/Webhook 入口（提交任务/查询状态） | 8h | P1 | T5.5 | ⬜ |
| T6.2 | 多模态扩展：支持文档附件（pdf/docx/txt）解析入上下文 | 8h | P1 | T5.3 | ⬜ |
| T6.3 | 运维体验：扩展高频配置热更新（减少重启需求） | 6h | P1 | T5.1 | ⬜ |

### 阶段 7：产品化增强（待开始）
| 任务ID | 任务描述 | 预估时间 | 优先级 | 依赖 | 状态 |
|--------|----------|----------|--------|------|------|
| T7.1 | 权限体系：按接口/动作/房间细粒度 RBAC | 8h | P2 | T6.1 | ⬜ |
| T7.2 | 历史能力：会话检索、导出与保留策略管理 | 8h | P2 | T6.1 | ⬜ |
| T7.3 | 策略自动化：按房间/用户/任务类型自动选择 backend/model | 8h | P2 | T6.1,T6.3 | ⬜ |
